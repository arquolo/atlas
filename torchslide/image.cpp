...
size_t Image::get_scale(const LevelInfo& info) const noexcept {
    return static_cast<size_t>(
        std::round(static_cast<double>(this->levels.at(0).shape.front()) /
                   static_cast<double>(info.shape.front())));
}

std::vector<size_t> Image::scales() const noexcept {
...
std::pair<Level, size_t> Image::get_level(size_t scale) const noexcept {
