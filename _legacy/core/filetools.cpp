...
#include "filetools.h"
    ...
    #include <windows.h>  // Note: we now need Windows 2000 or above
#else
    const std::string dirsep("/");
...
size_t file_size(const std::string& name) {
    if (!fileExists(name))
        return -1;
	size_t length = static_cast<size_t>(file_size(name));
	/*
...
std::string uniform_slashes(const std::string& path) {
