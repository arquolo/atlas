...
bool EXPORT is_file_exists(const std::string& name);

// Returns true iff the directory exists
bool EXPORT is_dir_exists(const std::string& name);

// Returns the size of the file in bytes.
// If the file does not exist or an error occurs, -1 is returned.
// Note: maximum file size is 2GB (legacy)
size_t EXPORT file_size(const std::string& name);

// Deletes the file, returns true iff the file existed and was successfully
// deleted.
bool EXPORT delete_file(const std::string& name);

// Deletes the directory, returns true if the directory existed, is empty,
// and was successfully deleted.
bool EXPORT delete_dir(
    const std::string& name, bool non_empty_ok = false);

// Returns true if the directory exists and is empty.
bool EXPORT is_empty_dir(const std::string& name);
...
std::string EXPORT extract_file_path(const std::string& name);
...
std::string EXPORT up_one_level(const std::string& name);

// Returns the level above the current directory structure level
// If a root directory is given it is returned unchanged
// If a file name is attached to the directory given then the filePath
// to the file is returned
std::string EXPORT up_multiple_levels(
    const std::string& name,
    size_t levels_count);
...
std::string EXPORT extract_file_name(const std::string& name);
...
std::string EXPORT extract_base_name(const std::string& name);
...
std::string EXPORT extract_lowest_dirname(const std::string& name);

// Returns the extension, if any, without the .
std::string EXPORT extract_file_extension(const std::string& name);

// Creates the directory, and all higher directories if necessary.
// Returns true iff the directory exists on exit.
bool EXPORT create_directory(const std::string& dir);
...
void EXPORT get_subdirectories(
    const std::string& path,
    std::vector<std::string>& v,
    bool recurse = false,
);
...
#ifndef LINUX
void EXPORT get_subdirectories_windows(
    const std::string& path,
    std::vector<std::string>& v,
    bool recurse,
);
...
bool EXPORT create_directory(const std::string& path);
...
std::string EXPORT root_name(const std::string& spath);
...
*/
std::string EXPORT get_path_relative_to_location(
    const std::string& path_to_alter,
...
bool EXPORT is_root(const std::string& spath);

// Returns true if the given path is a UNC path. This means it starts with \\.
bool EXPORT is_unc_path(const std::string& spath);
...
// hello
// !!!@
bool EXPORT is_only_directory(const std::string& spath);

bool EXPORT is_only_directory_tmp_keelin(const std::string& spath);
...
std::string EXPORT current_dir_path();
...
std::string EXPORT complete_path(
...
void EXPORT change_extension(
    std::string& name, const std::string& newextension);
...
void EXPORT change_basename(
    std::string& name, const std::string& newbasename);
...
void EXPORT changePath(
    std::string& name, const std::string& newpath);

////////////////////////////
// Reads the contents of a text file into s. Returns true iff file exists and
// was successfully read.
  bool EXPORT readFile(const std::string &filename, std::string &s);
...
// Returns true iff file exists and was successfully read.
  bool EXPORT readFile(const std::string &filename, std::vector<std::string> &vs);

  bool EXPORT readFileTail(const std::string &filename, std::vector<std::string> &vs, int nBytesToRead = 2048);
...
// for example when one of the paths is a network path and the other a local path).
  bool EXPORT equivalentPaths(const std::string path1,  const std::string path2);
...
void EXPORT get_date_time(std::string& s);
...
void EXPORT get_temp_dir(std::string& dirname);
...
// parent directory.
void EXPORT get_empty_temp_dir(
...
std::vector<std::string> EXPORT windows_drive_letters();

std::string EXPORT strip_trailing_slash(const std::string& str);

std::string EXPORT dir_separator();

bool EXPORT is_complete(const std::string& spath);

std::string EXPORT uniform_slashes(const std::string& path);
