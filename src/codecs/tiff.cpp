...
TIFFImage::TIFFImage(std::string& const imagepath) {
    std::unique_lock lk{open_close_mutex_};

#ifdef _WIN32
    int wchars_num = MultiByteToWideChar(CP_UTF8, 0, imagepath.c_str(), -1, NULL, 0);
    std::vector<wchar_t> w_imagepath(wchars_num);
    MultiByteToWideChar(CP_UTF8, 0, imagepath.c_str(), -1, w_imagepath.data(), wchars_num);
    tiff_ = TIFFOpenW(w_imagepath, "rm");
#else
    tiff_ = TIFFOpen(imagepath.c_str(), "rm");
#endif

    if (tiff_) {
    const char* img_desc = nullptr;

    if (TIFFGetFieldDefaulted(tiff_, TIFFTAG_IMAGEDESCRIPTION, &img_desc)) {
        std::string img_desc_string(img_desc);
        if (img_desc_string.find("DICOM") != std::string::npos
                || img_desc_string.find("xml") != std::string::npos
                || img_desc_string.find("XML") != std::string::npos)
    ...
    size_t ctype = 0;
    size_t dtype = 0;
    size_t planarconfig = 0;
    size_t bitsPerSample = 0;
    TIFFGetField(tiff_, TIFFTAG_PHOTOMETRIC, &ctype);
    ...
    if (TIFFGetField(tiff_, TIFFTAG_SAMPLEFORMAT, &dtype) == 0 && dtype == 0)
        dtype = 1;

    TIFFGetField(tiff_, TIFFTAG_SAMPLESPERPIXEL, &samples_per_pixel_);
    // Going to use TIFFReadRGBA for cType == PHOTOMETRIC_YCBCR, so four samples
    if (ctype == PHOTOMETRIC_YCBCR)
        samples_per_pixel_ = 4;
