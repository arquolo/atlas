...
bool VSIImage::initializeType(const std::string& imagePath) {
    ...
    core::getFiles(
        core::completePath("_" + baseName + "_", pth),
        "*.ets",
        etsFiles,
        true);
    if (etsFiles.empty()) {
        core::getFiles(
            core::completePath(baseName, pth), "*.ets", etsFiles, true);
        if (etsFiles.empty()) {
            core::getFiles(
                "_" + core::completePath(baseName, pth),
                "*.ets",
                etsFiles,
                true);
            if (etsFiles.empty()) {
                core::getFiles(
                    core::completePath(baseName, pth) + "_",
                    "*.ets",
                    etsFiles,
                    true);
                if (etsFiles.empty()) {
                    std::cout << "Could not find the .ets files belonging to "
                              << baseName
                              << ".vsi, is the VSI-folder present and "
                                 "correctly named?"
                              << std::endl;
                    return false;
                }
