...
static OPJ_SIZE_T opj_memory_stream_read(
    void* p_buffer, OPJ_SIZE_T p_nb_bytes, void* p_user_data) {
    opj_memory_stream* l_memory_stream
        = (opj_memory_stream*)p_user_data; //Our data.
    ...
    if (p_nb_bytes > (dataEndOffset - l_memory_stream->offset)) {
        l_nb_bytes_read
            = dataEndOffset - l_memory_stream->offset; //Read all we have.
    }

    //Copy the data to the internal buffer.
    memcpy(
        p_buffer, &(l_memory_stream->data[l_memory_stream->offset]),
        l_nb_bytes_read);
    l_memory_stream->offset
        += l_nb_bytes_read; //Update the pointer to the new location.
    return l_nb_bytes_read;
}

//This will write from the stream to p_buffer.
static OPJ_SIZE_T opj_memory_stream_write(
    void* p_buffer, OPJ_SIZE_T p_nb_bytes, void* p_user_data) {
    opj_memory_stream* l_memory_stream
        = (opj_memory_stream*)p_user_data; //Our data.
    OPJ_SIZE_T dataEndOffset = l_memory_stream->size - 1;
    ...
    if (p_nb_bytes > (dataEndOffset - l_memory_stream->offset)) {
        l_nb_bytes_write = dataEndOffset
            - l_memory_stream->offset; //Write the remaining space.
    }

    //Copy the data from the internal buffer.
    memcpy(
        &(l_memory_stream->data[l_memory_stream->offset]), p_buffer,
        l_nb_bytes_write);
    l_memory_stream->offset
        += l_nb_bytes_write; //Update the pointer to the new location.
    return l_nb_bytes_write;
}

//Moves the current offset forward, but never more than size.
static OPJ_OFF_T
opj_memory_stream_skip(OPJ_OFF_T p_nb_bytes, void* p_user_data) {
...
static OPJ_BOOL
opj_memory_stream_seek(OPJ_OFF_T p_nb_bytes, void* p_user_data) {
    opj_memory_stream* l_memory_stream = (opj_memory_stream*)p_user_data;
...
opj_stream_t* opj_stream_create_default_memory_stream(
    opj_memory_stream* p_memoryStream, OPJ_BOOL is_read_stream) {
    ...
    opj_stream_set_user_data(
        l_stream, p_memoryStream, opj_memory_stream_do_nothing);
...
void JPEG2000Codec::decode(
    ...
    opj_codec_t* decoder = opj_create_decompress(
        OPJ_CODEC_FORMAT::OPJ_CODEC_J2K); //Bad openjpeg, can't be reused!
    ...
    for (int index = 0; index < stream_len; index++) {
        for (unsigned int cmp = 0; cmp < decompImage->numcomps; cmp++) {
            for (int byteCnt = 0; byteCnt < bytes_jpc; byteCnt++) {
                *(buf)++
                    |= (unsigned char)((*compPointers[cmp] >> (8 * byteCnt)) & 0xFF);
            }
            *compPointers[cmp]++;
        }
    }
...
void JPEG2000Codec::encode(
    char* data,
    unsigned int& size,
    const unsigned int& tileSize,
    const unsigned int& rate,
    const unsigned int& nrComponents,
    const enums::Data& dataType,
    const enums::Color& colorSpace) const {

    int depth = 8;
    if (dataType == enums::Data::Float
        || dataType == enums::Data::Data::Invalid) {
        return;
    } else if (dataType == enums::Data::UInt16) {
        depth = 16;
    } else if (
        dataType == enums::Data::UInt32
        && colorSpace != enums::Color::ARGB) {
        depth = 32;
    }

    opj_cparameters_t encodeParameters; // compression parameters.
    encodeParameters.tcp_mct
        = nrComponents > 1 ? 1 : 0; //Decide if MCT should be used.
    ...
    opj_image_cmptparm_t* componentParameters
        = new opj_image_cmptparm_t[nrComponents];
    ...
    opj_image* encodedImage = opj_image_create(
        nrComponents, &componentParameters[0], jpegColorSpace);

    // Set image offset and reference grid
    encodedImage->x0 = 0;
    encodedImage->y0 = 0;
    encodedImage->x1 = (OPJ_UINT32)(componentParameters[0].w - 1)
            * (OPJ_UINT32)encodeParameters.subsampling_dx
        + 1;
    encodedImage->y1 = (OPJ_UINT32)(componentParameters[0].h - 1)
            * (OPJ_UINT32)encodeParameters.subsampling_dy
        + 1;

    // Setup the encoder parameters using the current image and user parameters.
    OPJ_BOOL setupSuccess
        = opj_setup_encoder(encoder, &encodeParameters, encodedImage);
    ...
    //encode the image.
    OPJ_BOOL startEncodeSuccess
        = opj_start_compress(encoder, encodedImage, l_stream);
    ...
    unsigned char* enc_bytes_ptr
        = encodingBuffer.data; // Pointer to start of encoded data
